...
| 2026-08 (M7) | **Wayland hot corner = input-region top strips** — one transparent 4 px top-edge layer surface per output; enter/leave crossing events are the corner trigger | Wayland has no global pointer-position API (§15); the strip *is* its own input region. Two hard-won mechanisms (handoff 07): (1) the strip needs an **exclusive zone** — without it wlroots places it in the layer's *free* area, below reserved chrome (the PIXEL bar), where 4 px was unhittable; with it, the strip lands at the monitor's true top edge, parity with X11. (2) the strip must **commit a buffer** to be mapped — an empty transparent window commits without attaching, so a DrawingArea forces a transparent ARGB buffer. Trade-off: the strip captures clicks in its 4 px band — the pointer never passes through there (acceptable on the TV/testing surfaces; X11 keeps the passive corner) |
...
- **M7 (layer-shell) landed:** on compositors with `zwlr_layer_shell_v1` (wlroots family —
  ...
  fullscreen by construction), placement via anchor + margins, hot corner via 4 px input-
