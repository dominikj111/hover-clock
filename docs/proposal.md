...
| 2026-08 | **The strict overlay contract is hover-clock policy, not shell architecture** | Never-focus / invisible-to-task-switchers / transparent-or-invisible activation are right for the quick-reveal clock use case, but must not be baked into the facade contracts (§10) as architecture. Future operational shells and kiosk apps may be fullscreen or windowed, may take focus (a focused dashboard should keep it — kiosks usually remove the taskbar anyway), may appear in the taskbar. Activation is a pluggable affordance: an invisible hot area is one option; a floating icon or a kiosk center-circle button are equally valid. Wayland's no-true-transparency (labwc composites layer surfaces opaque, §17.3) pushes toward explicit graphical affordances — a UX direction, not a limitation. hover-clock is exploratory: it validates the shell architecture and the backends, not the overlay constraints themselves |
